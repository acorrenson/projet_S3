...
def longtour_zero(t):
    ...
    for i in range(len(t)-1):
        ...
        l = l+problem.wfunc(t[i], t[i+1])
    l = l + \
        euclidean_2d((0, 0), (problem.get_display(t[0])[
                     0], problem.get_display(t[0])[1]))
    l = l + \
        euclidean_2d((problem.get_display(
            t[-1])[0], problem.get_display(t[-1])[1]), (0, 0))
    return l
...
def graphe(problem, tour, c):
    long = len(problem)
    plt.plot([problem.get_display(tour[i % long])[0] for i in range(long+1)],
             [problem.get_display(tour[i % long])[1] for i in range(long+1)], 'x'+c+'-')
    plt.show()
